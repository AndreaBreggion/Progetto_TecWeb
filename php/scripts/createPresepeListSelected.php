...
function createPresepePostSelected($row, $connection) {
...
function createPresepeListSelected($connection) {
    $query = 'SELECT presepi.*, users.username as username, users.id as UID from presepi INNER JOIN users on presepi.uId = users.id AND presepi.winner=1';
    $result = $connection->query($query);
    $returnValue = '';
    while($row = mysqli_fetch_assoc($result)) {
        $returnValue .= createPresepePostSelected($row, $connection);
    }
    return($returnValue);
}
?>